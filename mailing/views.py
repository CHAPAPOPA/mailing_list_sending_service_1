...
def home_page(request):
    ...
    data["newsletters"], data["active_newsletters"], data["clients"] = get_newsletters(
        request, data
    )
...
class NewsletterCreateView(
    LoginRequiredMixin, NotManagerMixin, UserAutofillMixin, KwargsMixin, CreateView
):
    model = Newsletter
...
class NewsletterUpdateView(
    LoginRequiredMixin, OwnerManagerMixin, KwargsMixin, UpdateView
):
    model = Newsletter
    form_class = NewsletterForm
...
class MessageCreateView(
    LoginRequiredMixin, NotManagerMixin, UserAutofillMixin, CreateView
):
    model = Message
    fields = ["subject", "text"]
